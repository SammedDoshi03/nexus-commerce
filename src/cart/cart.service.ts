...
export class CartService {
    ...
    async addToCart(userId: string, productId: string, quantity: number): Promise<Cart> {
        ...
        const updatedCart = await this.cartRepository.update((cart as any)._id, { items: cart.items });
    ...
    async clearCart(userId: string): Promise<Cart> {
        let cart = await this.getCart(userId);
        return this.cartRepository.update((cart as any)._id, { items: [] }) as Promise<Cart>;
    }
}
